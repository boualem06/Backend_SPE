...
def LdaImplementation():
    ...
    data[features_columns] = data[features_columns].applymap(
        lambda x: x.lower() if isinstance(x, str) else x
    )

    # Convert content of each row in targets columns to lowercase
    data[target_columns] = data[target_columns].applymap(
        lambda x: x.lower() if isinstance(x, str) else x
    )
    ...
    encoded_categorical_features = pd.get_dummies(
        features_df, columns=categorical_features.columns
    )

    # Combine the encoded features and numerical features
    final_features = pd.concat(
        [encoded_categorical_features, numerical_features], axis=1
    )
    ...
    for target_col in target_columns:
        ...
        target = target.fillna(
            -1
        )  # Replace NaN with a placeholder value, or choose an appropriate value
