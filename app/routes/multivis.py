...
def highest_sim_word(
    word_list,
):  # these function allows us to calculate the word that had the heighest similarities to the other words
    most_similar_word = min(
        word_list,
        key=lambda word: sum(levenshtein_distance(word, other) for other in word_list),
    )
    return most_similar_word


def calculating_similarities(
    data,
):  # these function allows us to calculate the similaire words
    ...
    for dictionary in data:
        ...
        similar_words_data.append(
            {"column_name": column_name, "response_counts": similar_words}
        )
...
# ************************************************************************************
def sum_similar_words(
    plt_count, arra_sim
):  # these function allows sum the values of similaire words together and take only the word theat have the max of similarities in these words
    final_res = []
    arr_word_sim = []
    temp_array = []
    simmm = []
    for index, elem in enumerate(plt_count):
        ...
        for word, count in response_counts.items():
            ...
            list_similaire_words = [
                w for w in list_similaire_words if w not in arr_word_sim
            ]
            ...
            if list_similaire_words:
                ...
                obj[elem["column_name"]].append(
                    {word_heighest_sim: list_similaire_words}
                )
                # simmm.append({word_heighest_sim:list_similaire_words})
                final_dic["values"].append(
                    {column_name: word_heighest_sim, "value": count}
                )
...
def send_graph(data_cloud):
    ...
    for col in data_cloud:
        ...
        for item in data:
            ...
            group_id = len(
                word_groups
            )  # Use the current number of groups as the group ID
            ...
            for word in similar_words:
                ...
                temp_obj[column_name].append(
                    {word: {"group_id": group_id, "x": x_start, "y": y_start}}
                )
...
def visualize_data_and_plot():
    try:
        # file_path = "./files/data2.csv"  # Replace with your actual file path
        ...
        for column_name in column_names:
            ...
            actual_column_name = [
                col for col in df.columns if desired_column_name.lower() in col.lower()
            ][0]
